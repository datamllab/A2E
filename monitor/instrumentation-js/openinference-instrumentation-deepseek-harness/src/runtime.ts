...
import { DeepSeekTraceMonitor } from "./monitor.js";

export interface DeepSeekMonitorEnvironment extends NodeJS.ProcessEnv {
  A2E_COLLECTOR_ENDPOINT?: string;
  A2E_PROJECT_NAME?: string;
  A2E_CLIENT_HEADERS?: string;
  A2E_API_KEY?: string;
  A2E_DEEPSEEK_CAPTURE_CONTENT?: string;
  A2E_DEEPSEEK_MAX_ATTRIBUTE_LENGTH?: string;
...
export function resolveRuntimeConfig(env: DeepSeekMonitorEnvironment = process.env): {
  endpoint: string;
  projectName: string;
  ...
  const baseEndpoint = env.A2E_COLLECTOR_ENDPOINT ?? env.OTEL_EXPORTER_OTLP_ENDPOINT
    ?? "http://127.0.0.1:6006";
  const endpoint = env.OTEL_EXPORTER_OTLP_TRACES_ENDPOINT
    ? env.OTEL_EXPORTER_OTLP_TRACES_ENDPOINT.replace(/\/+$/, "")
  ...
  const parsedLimit = Number(env.A2E_DEEPSEEK_MAX_ATTRIBUTE_LENGTH);
  return {
    endpoint,
    projectName: env.A2E_PROJECT_NAME || "default",
    headers,
    captureContent: enabled(env.A2E_DEEPSEEK_CAPTURE_CONTENT, true),
    maxAttributeLength:
      Number.isFinite(parsedLimit) && parsedLimit >= 1_024 ? parsedLimit : 262_144,
  };
}

export function extractParentContext(env: DeepSeekMonitorEnvironment): Context | undefined {
...
export function createA2EDeepSeekMonitor(
  env: DeepSeekMonitorEnvironment = process.env,
): DeepSeekTraceMonitor {
